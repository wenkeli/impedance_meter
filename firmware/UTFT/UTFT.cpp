...
#include "hardware/arm/HW_STM32F1.h"
...
{ 
	...
	{
//		P_RS	= &GPIOD->regs->ODR;//portOutputRegister(digitalPinToPort(RS));
//		B_RS	= 0x01<<11;//digitalPinToBitMask(RS);
//		P_WR	= &GPIOD->regs->ODR;//portOutputRegister(digitalPinToPort(WR));
//		B_WR	= 0x01<<5;//digitalPinToBitMask(WR);
//		P_CS	= &GPIOD->regs->ODR;//portOutputRegister(digitalPinToPort(CS));
//		B_CS	= 0x01<<7;//digitalPinToBitMask(CS);
//		P_RST	= &GPIOD->regs->ODR;//portOutputRegister(digitalPinToPort(RST));
//		B_RST	= 0x01<<6;//digitalPinToBitMask(RST);
...
uint16 UTFT::rgbTo16Bit(byte r, byte g, byte b)
{
	uint16 color;
	color=0;
	color=color|(((r&248)|g>>5)<<8);
	color=color|((g&28)<<3|b>>3);
